...
UINT count /* Number of sectors to read */
	...
	for (int i = 0; i < count; i++) {
		flashReadSector(buff, sector);
		sector++;
		buff += FLASH_ATTR_SECTOR_SIZE;
	}
	return RES_OK;
	/* USER CODE END READ */
}
...
//		W25X_Write_Sector(sector, (u8*) buff);
		...
		flashWriteSector((uint8_t*) buff, sector, 1);
...
void *buff /* Buffer to send/receive control data */
	...
	case GET_BLOCK_SIZE:
		*(DWORD*) buff = FLASH_ATTR_BLOCK_SIZE;
		break;

	case GET_SECTOR_SIZE:
		*(DWORD*) buff = FLASH_ATTR_SECTOR_SIZE;
		break;

	case GET_SECTOR_COUNT:
		*(DWORD*) buff = FLASH_ATTR_SECTOR_COUNT;
		break;
