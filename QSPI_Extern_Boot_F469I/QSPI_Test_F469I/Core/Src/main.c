...
int main(void) {
	...
	uint8_t recv[256] = { 0 };
	sprintf((char*) send, "%s",
			"This is a test file to see if we can write to the QSPI NOR flash\n This is a new line in the same write\n\0");

	CSP_QSPI_Write(send, 0, sizeof(send));

	CSP_QSPI_Read(recv, 0, 256);
	printf("%s\n", (char*) recv);
	printf("End\n");
