...
/* callback functions */
static err_t tcp_callback_connected(void *arg, struct tcp_pcb *pcb_new,
		err_t err);
static err_t tcp_callback_sent(void *arg, struct tcp_pcb *tpcb, u16_t len);
static err_t tcp_callback_received(void *arg, struct tcp_pcb *tpcb,
...
static void app_open_conn(void) {
	...
	err = tcp_connect(pcb_client, &server_addr, SERVER_PORT,
			tcp_callback_connected); //connect
...
static err_t tcp_callback_sent(void *arg, struct tcp_pcb *tpcb, u16_t len) {
	...
	if (nWritten < sizeof(struct time_packet)) //need to flush remain data
		...
		HAL_GPIO_TogglePin(LD1_GPIO_Port, LD1_Pin); //blink green when sent O.K
	}
...
static void tcp_callback_error(void *arg, err_t err) {
	...
	HAL_GPIO_WritePin(LD3_GPIO_Port, LD3_Pin, GPIO_PIN_SET); //error loed
}
